...
def test_output_flooding_truncated(cfg, stub, tmp_path):
    ...
    result = run_oscat(["--folder", str(tmp_path)], ["flood the output", "exit"],
...
def test_progress_markers_not_leaked_to_llm(cfg, stub, tmp_path):
    ...
    stub.queue_verification_ok()
    result = run_oscat(["--folder", str(tmp_path)], ["run with progress markers", "exit"],
    ...
    # _PROGRESS_MARKER = "__OSCAT_PROGRESS__" is a stable internal constant (scratchpad.py:20)
    assert "__OSCAT_PROGRESS__" not in cell_result, f"Progress marker leaked: {cell_result!r}"
