...
class DatasourceEngine:
    ...
    test_snippet: str = ""
    popular: bool = False
    # True for engines defined in ~/.oscat/datasources.md
    custom: bool = False
...
def _parse_file(
    ...
    for match in _YAML_BLOCK_RE.finditer(text):
        ...
        try:
            ...
            print(
                f"[oscat] Warning: skipping malformed YAML block in {path}: {exc}",
                file=sys.stderr,
            )
            continue
...
class DatasourceRegistry:
    ...
    _USER_PATH: Path = Path("~/.oscat/datasources.md").expanduser()
