...
CHAT_SYSTEM_PROMPT = """\
You are OSCAT — a self-evolving autonomous system that collaborates with people to \
...
(~/.oscat/memory/) and project (<workspace>/.oscat/memory/) scopes.
...
- All .oscat/.env variables are available as environment variables (os.environ).
...
code and never read ~/.oscat/data_vault/ files directly.
...
- Clipboard images are saved to .oscat/uploads/ — open with Pillow, OpenCV, etc.
...
# ---------------------------------------------------------------------------
# Visualization prompt variants — selected by OSCAT_PROACTIVE_DASHBOARDS flag
# ---------------------------------------------------------------------------
...
or similar. Write to `.oscat/output/name.html` and open in browser.
...
in the SAME directory (e.g. `.oscat/output/dashboard_data.js`) and reference it with a \
...
- Save output to `.oscat/output/` (create it if needed). Use descriptive filenames like \
...
Save to .oscat/output/. Do NOT auto-open the file from scratchpad code — instead call the \
