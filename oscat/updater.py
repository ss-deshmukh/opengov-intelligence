...
def check_and_update(console, settings) -> bool:
    """Check for a newer version of OSCAT and self-update if available.
...
def _check_and_update(result: dict, settings) -> None:
    ...
    try:
        local_ver = Version(oscat.__version__)
    ...
    messages.append(f"  Updating oscat {local_ver} \u2192 {remote_ver}...")

    try:
        proc = subprocess.run(
            ["uv", "tool", "upgrade", "oscat"],
            capture_output=True,
    ...
    try:
        ...
        # Look for "oscat X.Y.Z" in the output
        installed_match = re.search(r"oscat\s+(\S+)", tool_list)
        if installed_match:
