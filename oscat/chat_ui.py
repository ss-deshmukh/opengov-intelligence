...
class StreamDisplay:
    ...
    def _build_spinner_display(self) -> object:
        ...
        spinner = Spinner(
            "dots", text=Text(f" {self._line2_status}", style="oscat.muted")
        )
        ...
        if self._line3_peek:
            line3 = Text()
            line3.append("  \u21b3 ", style="oscat.muted")
    ...
    def finish(self) -> None:
        ...
        if self._initial_text and not self._initial_printed:
            if self._activities:
                self._console.print(
                    Text(self._initial_text.rstrip(), style="oscat.muted")
                )

        # Print answer
        if self._activities:
            if self._buffer:
                self._console.print(Text("oscat> ", style="oscat.prompt"), end="")
    ...
    def show_context_compacted(self, message: str) -> None:
        ...
        if not self._active:
            return
        self._stop_spinner()
        self._console.print(Text(f"> {message}", style="oscat.muted"))
        self._start_spinner()
    ...
    def _print_activity_line(self, act: _ToolActivity) -> None:
        ...
        if act.eta_str:
            line.append(f" {act.eta_str}", style="oscat.muted")
    ...
    def _print_done_line(self, act: _ToolActivity, elapsed: float) -> None:
        ...
        line.append(elapsed_str, style="oscat.muted")
        self._console.print(line)
...
class ClosingSpinner:
    ...
    def start(self) -> None:
        spinner = Spinner(
            "dots", text=Text(" Closing scratchpad processes…", style="oscat.muted")
        )
        self._live = Live(
