...
def _build_env_files() -> list[str]:
    """Build .env loading chain: cwd/.env -> .oscat/.env -> ~/.oscat/.env"""
    files: list[str] = [".env"]
    local_env = Path.cwd() / ".oscat" / ".env"
    if local_env.is_file():
        files.append(str(local_env))
    user_env = Path("~/.oscat/.env").expanduser()
    if user_env.is_file():
...
class OscatSettings(BaseSettings):
    model_config = {"env_prefix": "OSCAT_", "env_file": _ENV_FILES, "env_file_encoding": "utf-8", "extra": "ignore"}
    ...
    memory_dir: str = ".oscat"

    context_dir: str = ".oscat/context"
    ...
    # Analytics — anonymous usage events (set OSCAT_ANALYTICS_ENABLED=false to opt out)
    ...
    # Publish service (oscat-services API Gateway)
    ...
    def workspace_path(self) -> Path:
        """Return the resolved workspace root (parent of .oscat/)."""
