...
OSCAT reads this file when connecting data sources. For each source, the YAML
block defines the fields Python collects. The prose below describes auth flows,
common errors, and how to handle OAuth2 — OSCAT handles those using the scratchpad.
...
Follow the YAML format above. Add to `~/.oscat/datasources.md` (user overrides).
OSCAT merges user overrides on top of the built-in registry at startup.
