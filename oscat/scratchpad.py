...
_MAX_OUTPUT = 10_000
_PROGRESS_MARKER = "__OSCAT_PROGRESS__"
...
_CELL_DELIM = "__OSCAT_CELL_END__"
_RESULT_START = "__OSCAT_RESULT__"
_RESULT_END = "__OSCAT_RESULT_END__"
...
class Scratchpad:
    ...
    _venvs_base: Path = field(
        default_factory=lambda: Path("~/.oscat/scratchpad-venvs").expanduser(),
        repr=False,
    )
    ...
    def _add_windows_firewall_rule(self) -> None:
        ...
        if self._venv_python is None or not os.path.isfile(self._venv_python):
            return
        import subprocess as _sp
        rule_name = f"OSCAT Scratchpad - {self.name}"
        try:
            _sp.run(
                [
    ...
    async def start(self) -> None:
        ...
        fd, path = tempfile.mkstemp(suffix=".py", prefix="oscat_scratchpad_")
        ...
        if self._coding_model:
            env["OSCAT_SCRATCHPAD_MODEL"] = self._coding_model
        if self._coding_provider:
            env["OSCAT_SCRATCHPAD_PROVIDER"] = self._coding_provider
        # Ensure the SDKs can find API keys under their expected names.
        # OSCAT stores them as OSCAT_*_API_KEY; the SDKs expect *_API_KEY.
        if "ANTHROPIC_API_KEY" not in env and "OSCAT_ANTHROPIC_API_KEY" in env:
            env["ANTHROPIC_API_KEY"] = env["OSCAT_ANTHROPIC_API_KEY"]
        if "OPENAI_API_KEY" not in env and "OSCAT_OPENAI_API_KEY" in env:
            env["OPENAI_API_KEY"] = env["OSCAT_OPENAI_API_KEY"]
        if "OPENAI_BASE_URL" not in env and "OSCAT_OPENAI_BASE_URL" in env:
            env["OPENAI_BASE_URL"] = env["OSCAT_OPENAI_BASE_URL"]
        # Minds credentials can serve as OpenAI-compatible fallback when
        # OSCAT_OPENAI_* vars aren't persisted (new clean config path).
        if "OPENAI_API_KEY" not in env and "OSCAT_MINDS_API_KEY" in env and self._coding_provider == "openai-compatible":
            env["OPENAI_API_KEY"] = env["OSCAT_MINDS_API_KEY"]
        if "OPENAI_BASE_URL" not in env and "OSCAT_MINDS_URL" in env and self._coding_provider == "openai-compatible":
            env["OPENAI_BASE_URL"] = f"{env['OSCAT_MINDS_URL'].rstrip('/')}/api/v1"
        # If settings provided an explicit API key (e.g. from ~/.oscat/.env or
        ...
        # settings takes top priority, then OSCAT_OPENAI_BASE_URL from .env.
        if self._coding_provider in ("openai", "openai-compatible"):
            base_url = (
                self._coding_base_url
                or env.get("OSCAT_OPENAI_BASE_URL")
                or env.get("OPENAI_BASE_URL")
                or ""
            )
            if base_url:
                env["OPENAI_BASE_URL"] = base_url
                env["OSCAT_OPENAI_BASE_URL"] = base_url
        # Pass uv path so the boot script can use it for auto-installing
        # missing modules (same installer that created the venv).
        uv = self._find_uv()
        if uv:
            env["OSCAT_UV_PATH"] = uv

        # Ensure the oscat package is importable in the subprocess (needed for
        # get_llm and skill loading). The boot script runs from a temp file, so
        # the project root isn't on sys.path by default.
        _oscat_root = str(Path(__file__).resolve().parent.parent)
        python_path = env.get("PYTHONPATH", "")
        if _oscat_root not in python_path:
            env["PYTHONPATH"] = _oscat_root + (os.pathsep + python_path if python_path else "")
...
class ScratchpadManager:
    ...
    def __init__(
        ...
        if workspace_path is not None:
            self._venvs_base = workspace_path / ".oscat" / "scratchpad-venvs"
        else:
            self._venvs_base = Path("~/.oscat/scratchpad-venvs").expanduser()
