...
from oscat.llm.prompts import CHAT_SYSTEM_PROMPT, build_visualizations_prompt
...
from oscat.checks import TokenLimitInfo, TokenLimitStatus, check_minds_token_limits
from oscat.commands.setup import (
    handle_memory,
    handle_setup,
    handle_setup_memory,
    handle_setup_models,
)
from oscat.commands.ui import handle_theme, print_slash_help
from oscat.utils.clipboard import (
    ensure_clipboard,
    format_clipboard_image_message,
    format_file_message,
    human_size,
)
from oscat.chat_session import build_runtime_context, rebuild_session
from oscat.commands.session import handle_resume
...
from oscat.utils.prompt import (
    MINDS_KEYS,
    LLM_KEYS,
    SECRET_PATTERNS,
    mask_secret,
...
from oscat.minds_client import (
    normalize_minds_url,
...
from oscat.data_vault import DataVault
...
if TYPE_CHECKING:
    ...
    from oscat.memory.episodes import EpisodicMemory
...
class ChatSession:
    ...
    async def _build_system_prompt(self, user_message: str = "") -> str:
        ...
        if self._cortex is not None:
            ...
            if sa_section:
                prompt += sa_section
        # Inject oscat.md project context (user-written takes priority)
        if self._workspace is not None:
            md_context = self._workspace.build_oscat_md_context()
            if md_context:
                prompt += md_context
    ...
    def _build_tools(self) -> list[dict]:
        ...
        if self._cortex is not None:
            ...
            # Legacy fallback
            from oscat.tools import MEMORIZE_TOOL as _MT
                    ...
                    try:
                        if tc.name == "scratchpad" and tc.input.get("action") == "exec":
                            ...
                            if isinstance(prep, str):
                                ...
                                from oscat.scratchpad import Cell
...
async def _handle_connect(
    ...
    from oscat.workspace import Workspace as _Workspace
    ...
    minds_url = await prompt_or_cancel("(oscat) Minds server URL", default=saved_url)
    ...
    if not api_key:
        console.print("[oscat.error]API key is required.[/]")
        console.print()
        return session
    ...
    while minds is None:
        console.print()
        console.print(f"[oscat.muted]Connecting to {minds_url}...[/]")
        try:
            minds = list_minds(minds_url, api_key, verify=ssl_verify)
            break
        except (urllib.error.URLError, urllib.error.HTTPError) as err:
            headline, advice = describe_minds_connection_error(err)
            console.print(f"[oscat.error]{headline}[/]")
            console.print(f"[oscat.muted]{advice}[/]")
        ...
        action = await prompt_or_cancel("(oscat) Select", choices=["1", "2", "q"], default="q")
        if action is None or action == "q":
            console.print("[oscat.muted]Aborted.[/]")
            console.print()
            return session
        if action == "1":
            ...
            if new_key is None:
                console.print("[oscat.muted]API key unchanged.[/]")
                continue
            api_key = new_key
            ssl_verify = settings.minds_ssl_verify
            continue
    ...
    if not minds:
        console.print("[oscat.warning]No minds found on this server.[/]")
        console.print()
        return session

    # --- Select a Mind ---
    console.print()
    console.print("[oscat.cyan]Available minds:[/]")
    ...
    pick = await prompt_or_cancel("(oscat) Select mind", choices=choices)
    if pick is None:
        return session
    ...
    if len(mind_datasources) > 1:
        console.print()
        console.print(f"[oscat.cyan]Datasources in mind '{mind_name}':[/]")
        ...
        ds_pick = await prompt_or_cancel("(oscat) Select datasource", choices=ds_choices)
        if ds_pick is None:
            return session
        ...
        ds_name = picked_ds if isinstance(picked_ds, str) else picked_ds.get("name", "")
        console.print(f"[oscat.muted]Auto-selected datasource: {ds_name}[/]")
    ...
    # --- Persist to global ~/.oscat/.env ---
    global_ws.set_secret("OSCAT_MINDS_API_KEY", api_key)
    global_ws.set_secret("OSCAT_MINDS_URL", minds_url)
    global_ws.set_secret("OSCAT_MINDS_MIND_NAME", mind_name)
    global_ws.set_secret("OSCAT_MINDS_DATASOURCE", ds_name)
    global_ws.set_secret("OSCAT_MINDS_DATASOURCE_ENGINE", ds_engine)
    global_ws.set_secret("OSCAT_MINDS_SSL_VERIFY", "true" if ssl_verify else "false")
    ...
    console.print()
    status = f"[oscat.success]Selected mind: {mind_name}[/]"
    if ds_name:
        status += f" [oscat.success]| datasource: {ds_name} ({ds_engine})[/]"
    ...
    if llm_ok:
        console.print(
            "[oscat.success]LLM endpoints available — using Minds server as LLM provider.[/]"
        )
        ...
        global_ws.set_secret("OSCAT_PLANNING_PROVIDER", "openai-compatible")
        global_ws.set_secret("OSCAT_CODING_PROVIDER", "openai-compatible")
        global_ws.set_secret("OSCAT_PLANNING_MODEL", "_reason_")
        global_ws.set_secret("OSCAT_CODING_MODEL", "_code_")
    else:
        ...
        if not has_anthropic:
            ...
            if anthropic_key.strip():
                ...
                global_ws.set_secret("OSCAT_ANTHROPIC_API_KEY", anthropic_key)
                global_ws.set_secret("OSCAT_PLANNING_PROVIDER", "anthropic")
                global_ws.set_secret("OSCAT_CODING_PROVIDER", "anthropic")
                global_ws.set_secret("OSCAT_PLANNING_MODEL", "claude-sonnet-4-6")
                global_ws.set_secret("OSCAT_CODING_MODEL", "claude-haiku-4-5-20251001")
                console.print("[oscat.success]Anthropic API key saved.[/]")
            else:
                console.print(
                    "[oscat.warning]No API key provided — LLM calls will not work.[/]"
                )
...
async def _handle_publish(
    ...
    from oscat.publisher import publish

    console.print()

    # 1. Ensure Minds API key is available
    if not settings.minds_api_key:
        console.print("  [oscat.muted]To publish dashboards you need a free Minds account.[/]")
        console.print()
        ...
        settings.minds_api_key = api_key
        if workspace:
            workspace.set_secret("OSCAT_MINDS_API_KEY", api_key)
        console.print()

    # 2. Find the HTML file to publish
    import re

    output_dir = Path(settings.workspace_path) / ".oscat" / "output"
    ...
    if file_arg:
        ...
        if not html_files:
            console.print("  [oscat.warning]No HTML files found in .oscat/output/[/]")
            console.print()
            return
        ...
        while True:
            ...
            console.print("  [oscat.cyan]Available reports:[/]")
            console.print()
            for i, f in enumerate(page, offset + 1):
                title = _extract_html_title(f, re)
                label = title or f.name
                console.print(f"  [bold]{i}[/]  {label}  [oscat.muted]{f.name}[/]")

            if has_more:
                console.print(f"\n  [oscat.muted]m  Show more ({len(html_files) - offset - PAGE_SIZE} remaining)[/]")
            ...
            try:
                ...
                console.print("  [oscat.warning]Invalid choice.[/]")
                console.print()
                return

    if not target.exists():
        console.print(f"  [oscat.warning]File not found: {target}[/]")
        console.print()
        return
    ...
    with Live(Spinner("dots", text="  Publishing...", style="oscat.cyan"), console=console, transient=True):
        try:
            result = publish(
                target,
            ...
            console.print(f"  [oscat.error]Publish failed: {e}[/]")
            console.print()
            return

    view_url = result.get("view_url", "")
    console.print(f"  [oscat.success]Published![/]")
...
async def _handle_unpublish(
    ...
    from oscat.publisher import list_published, unpublish

    console.print()

    # 1. Ensure Minds API key is available
    if not settings.minds_api_key:
        console.print("  [oscat.warning]No Minds API key configured. Run /publish first.[/]")
        console.print()
        return
    ...
    reports = []
    with Live(Spinner("dots", text="  Loading published reports...", style="oscat.cyan"), console=console, transient=True):
        try:
            ...
            console.print(f"  [oscat.error]Failed to list reports: {e}[/]")
            console.print()
            return

    if not reports:
        console.print("  [oscat.muted]No published reports found.[/]")
        console.print()
        return
    ...
    while True:
        ...
        console.print("  [oscat.cyan]Published reports:[/]")
        console.print()
        for i, r in enumerate(page, offset + 1):
            title = r.get("title", "Untitled")
            url = r.get("view_url", "")
            console.print(f"  [bold]{i}[/]  {title}  [oscat.muted]{url}[/]")

        if has_more:
            console.print(f"\n  [oscat.muted]m  Show more ({len(reports) - offset - PAGE_SIZE} remaining)[/]")
    ...
    console.print(f"  [oscat.warning]This will remove:[/] {title}")
    ...
    # 5. Delete
    with Live(Spinner("dots", text="  Removing...", style="oscat.cyan"), console=console, transient=True):
        try:
            unpublish(
                selected["md5"],
                api_key=settings.minds_api_key,
            ...
            console.print(f"  [oscat.error]Failed to remove: {e}[/]")
            console.print()
            return

    console.print(f"  [oscat.success]Removed:[/] {title}")
    console.print()
...
async def _agent_zero(console: Console, session: "ChatSession", settings) -> str | None:
    ...
    answer = await prompt_or_cancel(
        "(oscat) Run analysis, or skip straight to chatting?",
    ...
    if not wants_demo:
        console.print()
        console.print("  [oscat.muted]All good! Ask me anything \u2014 data questions, dashboards, analysis, you name it.[/]")
        console.print()
        return None

    # Typed message with ellipsis animation
    console.print()
    from oscat.channel.theme import get_palette as _gp3
    _c = _gp3().cyan
    _r, _g, _b = int(_c[1:3], 16), int(_c[3:5], 16), int(_c[5:7], 16)
    _ac = f"\033[1;38;2;{_r};{_g};{_b}m"
    _ar = "\033[0m"

    _prefix = f"{_ac}oscat>{_ar} "
    ...
    with Live(
        Spinner("dots", text=Text("", style="oscat.muted"), style="oscat.cyan"),
        console=console,
    ...
    # 2. Override OUTPUT_PATH to write to .oscat/output/ instead of demo_data/
    code = script_path.read_text()
    output_dir = str(Path(settings.workspace_path) / ".oscat" / "output")
    ...
    # Replace the OUTPUT_PATH line so the dashboard goes to .oscat/output/
    code = code.replace(
    ...
    install_spinner = Text("  Installing dependencies (yfinance, pandas, numpy)...", style="oscat.muted")
    with Live(
        Spinner("dots", text=install_spinner, style="oscat.cyan"),
        console=console,
        refresh_per_second=10,
        transient=True,
    ):
        await pad.install_packages(["yfinance", "pandas", "numpy"])
    console.print(f"  [oscat.success]\u2714[/] [oscat.muted]Dependencies ready[/]")

    spinner_text = Text("  Scratchpad(Building NVDA vs BTC dashboard...)", style="oscat.muted")
    cell = None
    with Live(
        Spinner("dots", text=spinner_text, style="oscat.cyan"),
        console=console,
        refresh_per_second=10,
        transient=True,
    ):
        async for item in pad.execute_streaming(
            ...
            if isinstance(item, str):
                ...
                spinner_text = Text(f"  Scratchpad({item})", style="oscat.muted")
            elif isinstance(item, Cell):
                cell = item
    ...
    if cell is None or cell.error:
        ...
        console.print(f"[oscat.error]  Demo encountered an issue: {err_line}[/]")
        console.print("[oscat.muted]  You can still use OSCAT normally.[/]")
        console.print()
        return None

    console.print(f"  [oscat.success]\u2714[/] [oscat.muted]Dashboard built successfully[/]")
    ...
    from oscat.channel.theme import get_palette as _gp2
    _cyan = _gp2().cyan
    ...
    for li, line in enumerate(_lines):
        ...
        console.file.flush()
    console.print()
    console.print("[oscat.muted] Ask me follow-ups, a completely different question, or connect your own data (using the /connect command).[/]")
    console.print("[oscat.muted] What\u2019s next, boss?[/]")
    console.print()
...
def _persist_first_run_done(settings) -> None:
    """Write OSCAT_FIRST_RUN_DONE=true to ~/.oscat/.env."""
    from pathlib import Path

    env_path = Path.home() / ".oscat" / ".env"
    env_path.parent.mkdir(parents=True, exist_ok=True)
    existing = env_path.read_text() if env_path.is_file() else ""
    if "OSCAT_FIRST_RUN_DONE" not in existing:
        with env_path.open("a") as f:
            if existing and not existing.endswith("\n"):
                f.write("\n")
            f.write("OSCAT_FIRST_RUN_DONE=true\n")
...
def _desktop_greeting(console: Console, settings) -> None:
    ...
    from oscat.channel.theme import get_palette as _gp
    ...
    # Line 1: "Hi Boss! I'm OSCAT — here to help with anything."
    _line1 = "Hi Boss! I\u2019m OSCAT \u2014 here to help with anything."
    console.file.write(f"{_ac}oscat>{_ar} ")
    for ch in _line1:
        console.file.write(ch)
        console.file.flush()
        _time.sleep(0.02)
...
def run_chat(
    console: Console, settings: OscatSettings, *, resume: bool = False, first_run: bool = False, desktop_first_run: bool = False
) -> None:
...
async def _chat_loop(
    ...
    # Workspace for oscat.md and secret vault
    ...
    global_memory_dir = Path.home() / ".oscat" / "memory"
    project_memory_dir = settings.workspace_path / ".oscat" / "memory"
    ...
    project_oscat_dir = settings.workspace_path / ".oscat"
    if needs_reconsolidation(project_oscat_dir):
        actions = reconsolidate(project_oscat_dir)
        if actions:
            console.print(f"[oscat.muted]  Memory migration: {actions[0]}[/]")
    ...
    episodes_dir = settings.workspace_path / ".oscat" / "episodes"
    ...
    uploads_dir = Path(settings.workspace_path) / ".oscat" / "uploads"
    ...
    console.print("[oscat.muted] type '/help' for commands or 'exit' to quit.[/]")
    console.print()
    ...
    from oscat.chat_ui import StreamDisplay, EscapeWatcher, ClosingSpinner
    ...
    try:
        while True:
            ...
            if session._pending_memory_confirmations:
                ...
                console.print("[oscat.muted]Lessons learned from this session:[/]")
                ...
                if confirm in ("y", "yes"):
                    ...
                    console.print("[oscat.muted]Saved.[/]")
                elif confirm in ("n", "no"):
                    console.print("[oscat.muted]Discarded.[/]")
                else:
                    ...
                    try:
                        ...
                        if selected and cortex is not None:
                            ...
                            console.print(
                                f"[oscat.muted]Saved {len(selected)} entries.[/]"
                            )
                        else:
                            console.print("[oscat.muted]Discarded.[/]")
                    except (ValueError, IndexError):
            ...
            if not stripped:
                if is_clipboard_supported():
                    ...
                    if clip.image:
                        ...
                        console.print(
                            f"  [oscat.muted]attached: clipboard image "
            ...
            if message_content is None and stripped.startswith("/"):
                ...
                if cmd == "/llm":
                    ...
                    if not arg:
                        console.print(
                            "[oscat.warning]Usage: /edit <engine-name>[/]"
                        )
                        console.print()
                    else:
                        session = await handle_connect_datasource(
                    ...
                    if clip.image:
                        ...
                        console.print("[oscat.warning]No image found on clipboard.[/]")
                        continue
                else:
                    console.print(f"[oscat.warning]Unknown command: {cmd}[/]")
                    continue
            ...
            _total_questions += 1
            if _query_count == 1:
                send_event(settings, "oscat_first_query")
            else:
                send_event(settings, "oscat_query")
            ...
            try:
                ...
                if settings.minds_api_key and settings.minds_url and last_token_status is not None and last_token_status.status is TokenLimitStatus.WARNING:
                    ...
                    console.print(
                        f"[oscat.warning]Approaching token limit: {last_token_status.used:,} / "
                        f"{last_token_status.limit:,} tokens used ({pct}%). "
                        "Visit mdb.ai to upgrade your plan or top up your tokens.[/]"
                    )
                    console.print()
                if _query_count == 1:
                    send_event(settings, "oscat_first_answer")
            except anthropic.AuthenticationError:
                display.abort()
                console.print()
                console.print(
                    "[oscat.error]Invalid API key. Let's set up a new one.[/]"
                )
                settings.anthropic_api_key = None
                from oscat.cli import _ensure_api_key
                ...
                if session._scratchpads.list_pads():
                    ...
                    console.print()
                console.print("[oscat.muted]Cancelled.[/]")
                console.print()
                ...
                display.abort()
                console.print()
                console.print(f"[oscat.warning]{exc}[/]")
                console.print()
                choice = await prompt_or_cancel(
                    "  (oscat) Switch LLM provider, update API key, or retry?",
                ...
                display.abort()
                console.print()
                console.print(f"[oscat.error]{exc}[/]")
                console.print()
                choice = await prompt_or_cancel(
                    "  (oscat) Switch LLM provider, or retry?",
                    choices=["setup", "retry", "s", "r"],
                    choices_display="setup/retry",
                    default="retry",
                )
    ...
    console.print()
    console.print("[oscat.muted]See you.[/]")
    await session.close()
