...
def handle_memory(
    ...
    console.print()
    console.print("[oscat.cyan]Memory Status[/]")
    console.print()

    mode_labels = {
        "autopilot": "Autopilot — OSCAT decides what to remember",
    ...
    if cortex is None:
        console.print("  [oscat.warning]Memory system not initialized.[/]")
        console.print()
        return
    ...
    def _show_scope(label: str, hc) -> int:
        ...
        console.print(f"  [oscat.cyan]{label}[/] [dim]({hc._dir})[/]")
        if identity:
            entries = [
                ln.strip()[2:]
    ...
    if cortex.needs_compaction():
        console.print("  [oscat.warning]Compaction needed (>50 entries in a scope)[/]")
    console.print()

    if episodic is not None:
        status = "[bold]ON[/]" if episodic.enabled else "[dim]OFF[/]"
        sessions = episodic.session_count()
        console.print(f"  [oscat.cyan]Episodic Memory[/]")
...
async def handle_setup_memory(
    ...
    console.print()
    console.print("[oscat.cyan]Memory configuration[/]")
    console.print()

    console.print("  Memory mode:")
    console.print(
        r"    [bold]1[/]  Autopilot — OSCAT decides what to remember       [dim]\[recommended][/]"
    )
    console.print(
    ...
    mode_choice = await prompt_or_cancel(
        "(oscat) Memory mode", choices=["1", "2", "3"], default=current_mode_num
    )
    ...
    workspace.set_secret("OSCAT_MEMORY_MODE", memory_mode)
    ...
    if episodic is not None:
        ...
        toggle = await prompt_or_cancel(
            "(oscat) Toggle episodic memory?", choices=["y", "n"], default="n"
        )
        if toggle is None:
            toggle = "n"
        if toggle == "y":
            ...
            workspace.set_secret(
                "OSCAT_EPISODIC_MEMORY", "true" if new_state else "false"
            )
            console.print(f"  Episodic memory: [bold]{'ON' if new_state else 'OFF'}[/]")

    console.print()
    console.print("[oscat.success]Configuration updated.[/]")
    console.print()
...
async def handle_setup_models(
    ...
    from oscat.cli import _SetupRetry, _setup_minds, _setup_other_provider
    from oscat.chat_session import rebuild_session

    # Always persist API keys and model settings to global ~/.oscat/.env
    ...
    console.print()
    console.print("[oscat.cyan]Current configuration:[/]")
    ...
    def _print_choices():
        console.print("  [bold]1[/]  [link=https://mdb.ai][oscat.cyan]Minds-Enterprise-Cloud[/][/link] [oscat.success](recommended)[/]")
        console.print("  [bold]2[/]  [oscat.cyan]Minds-Enterprise-Server[/] [oscat.muted]self-hosted[/]")
        console.print("  [bold]3[/]  [oscat.cyan]Bring your own key[/] [oscat.muted]Anthropic / OpenAI / Gemini[/]")
        console.print("  [bold]q[/]  [oscat.muted]Back[/]")
        console.print()
...
async def handle_setup(
    ...
    console.print()
    console.print("[oscat.cyan]/setup[/]")
    console.print()
    ...
    top_choice = await prompt_or_cancel(
        "(oscat) Select", choices=["1", "2", "q"], default="q"
    )
