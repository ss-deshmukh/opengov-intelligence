...
from oscat.datasource_registry import DatasourceEngine, DatasourceField, DatasourceRegistry
...
def handle_list_data_sources(console: Console) -> None:
    ...
    console.print()
    if not conns:
        console.print("[oscat.muted]No data sources connected yet.[/]")
        console.print("[oscat.muted]Use /connect to add one.[/]")
        console.print()
        return
...
async def handle_remove_data_source(console: Console, slug: str) -> None:
    ...
    if not slug:
        ...
        if not connections:
            console.print("[oscat.muted]No saved connections to remove.[/]")
            console.print()
            return
        console.print()
        console.print("[oscat.cyan](oscat)[/] Which connection do you want to remove?\n")
        ...
        pick = await prompt_or_cancel("(oscat) Enter a number", choices=choices)
        if pick is None:
    ...
    if parsed is None:
        console.print(
            f"[oscat.warning]Invalid name '{slug}'. Use engine-name format.[/]"
        )
        console.print()
        return
    engine, name = parsed
    if vault.load(engine, name) is None:
        console.print(f"[oscat.warning]No connection '{slug}' found.[/]")
        console.print()
        return

    confirm = await prompt_or_cancel(
        f"(oscat) Remove '{slug}' from Local Vault?",
        ...
        if engine_def is not None and engine_def.custom:
            ...
            if not remaining:
                ...
                if user_path.is_file():
                    ...
                    registry.reload()
        console.print(f"[oscat.success]Removed {slug}.[/]")
    else:
        console.print("[oscat.muted]Cancelled.[/]")
    console.print()
...
async def show_credential_help(
    ...
    storage_note = (
        "The credentials will be stored securely in OSCAT's Local Vault — "
    ...
    if current_field is not None:
        ...
        heading = f"[oscat.cyan](oscat)[/] How to get [bold]{current_field.name}[/]:"
    else:
        prompt = (
        ...
        heading = f"[oscat.cyan](oscat)[/] How to get credentials for [bold]{service_name}[/]:"

    console.print()
    console.print("[oscat.muted]        Looking up instructions…[/]")
...
async def run_connection_test(
    ...
    while True:
        console.print()
        console.print("[oscat.cyan](oscat)[/] Got it. Testing connection…")
        ...
        try:
            ...
            if engine_def.pip:
                ...
                if "failed" in (install_result or "").lower():
                    console.print()
                    console.print(f"[oscat.warning](oscat)[/] Package install issue: {install_result[:200]}")
        ...
        if cell.error or (cell.stdout.strip() != "ok" and cell.stderr.strip()):
            ...
            console.print()
            console.print("[oscat.warning](oscat)[/] ✗ Connection failed.")
            console.print()
            console.print(f"        Error: {last_line}")
            console.print()
            retry = await prompt_or_cancel(
                "(oscat) Would you like to re-enter your credentials?",
                choices=["y", "n"], default="n",
            )
            ...
            for f in retry_fields:
                if not f.secret:
                    continue
                value = await prompt_or_cancel(f"(oscat) {f.name}", password=True)
                if value is None:
                    return False
                if value:
        ...
        console.print("[oscat.success]        ✓ Connected successfully![/]")
        return True
    ...
    *,
    ...
    if name:
        ...
        tool_name = await prompt_or_cancel(
            "(oscat) What is the name of the tool or service?",
        )
    ...
    if known_service:
        ...
        user_answer = ""
        console.print("[oscat.muted]        Working out the connection details…[/]")
    else:
        user_answer = await prompt_or_cancel(
            f"(oscat) How do you authenticate with {tool_name}? "
            "Describe what credentials you have (don't paste actual values)",
        )
        if not user_answer or not user_answer.strip():
            return None
        console.print()
        console.print("[oscat.muted]    Got it — working out the connection details…[/]")
    ...
    try:
        ...
        console.print(
            "[oscat.warning]        Couldn't identify connection details. Try again.[/]"
        )
        console.print()
        return None
    ...
    if not fields:
        console.print("[oscat.warning]    Couldn't identify any connection fields.[/]")
        console.print()
        return None
    ...
    for f, raw in zip(fields, raw_fields):
        ...
        if f.secret and inline_value:
            ...
            val_display = inline_value or "[oscat.muted]<to be collected>[/]"
    ...
    help_answer = await prompt_or_cancel(
        "(oscat) Do you need instructions on how to obtain these credentials?",
        choices=["y", "n"], default="n",
    )
    ...
    for f, raw in zip(fields, raw_fields):
        ...
        if f.name in credentials:
            continue
        value = await prompt_or_cancel(f"(oscat) {f.name}")
        if value is None:
            return None
        if value:
            credentials[f.name] = value
    ...
    for f, raw in zip(fields, raw_fields):
        if f.secret or f.required or f.name in credentials:
            continue
        value = await prompt_or_cancel(f"(oscat) {f.name} (optional — press Enter to skip)")
        if value is None:
            return None
        if value:
            credentials[f.name] = value
    ...
    if not credentials:
        console.print("[oscat.warning]        No credentials collected. Aborting.[/]")
        console.print()
        return None

    # Build engine slug and write definition to ~/.oscat/datasources.md
    ...
    user_ds_path = Path("~/.oscat/datasources.md").expanduser()
    ...
    if slug in parsed:
        ...
        console.print(
            "[oscat.warning]Could not validate engine definition — "
    ...
    if missing_required:
        console.print(
            "[oscat.warning]    Cannot save — missing required fields: "
...
async def handle_connect_datasource(
    ...
    if datasource_name is not None:
        ...
        if parsed is None:
            console.print(
                f"[oscat.warning]Invalid slug '{datasource_name}'. "
        ...
        if existing is None:
            console.print(
                f"[oscat.warning]No connection '{datasource_name}' found in Local Vault.[/]"
            )
            console.print()
            return session
        ...
        if engine_def is None:
            console.print(
                f"[oscat.warning]Unknown engine '{edit_engine}'. "
        ...
        console.print()
        console.print(
            f"[oscat.cyan](oscat)[/] Editing [bold]\"{datasource_name}\"[/bold]"
            f" ({engine_def.display_name})."
        )
        console.print("[oscat.muted]        Press Enter to keep the current value.[/]")
        console.print()
        ...
        console.print()
        console.print(
            "[oscat.muted]        You can now ask me questions about your data.[/]"
        )
        console.print()
    ...
    def print_sections() -> None:
        console.print(
            "[oscat.cyan](oscat)[/] Choose a data source:\n"
        )
    ...
    def print_all() -> None:
        console.print(
            "[oscat.cyan](oscat)[/] All data sources (★ = popular):\n"
        )
        console.print("       [bold]  Primary")
        console.print(
            "         [bold]  0.[/bold] Custom datasource"
    ...
    if prefill:
        ...
        print_sections()
        console.print(
            "       [oscat.muted]Don't see yours? Type a datasource name (e.g., GitHub, Gmail, Jira, ...)\n"
            "       It can be virtually any datasource — we'll figure out the details together.[/]"
        )
        console.print()
        answer = await prompt_or_cancel(
            "(oscat) Enter a number or type a datasource name",
        )
        if answer is None:
            return session
    ...
    if stripped_answer in known_slugs:
        ...
        console.print()
        console.print(
            f'[oscat.success]        ✓ Reconnected to [bold]"{stripped_answer}"[/bold].[/]'
        )
        console.print()
        session._history.append(
            {
    ...
    if stripped_answer.isdigit() or (stripped_answer.lstrip("-").isdigit()):
        ...
        if pick_num == 0:
            ...
            console.print(
                f"[oscat.warning](oscat)[/] '{stripped_answer}' is out of range. "
    ...
    if engine_def is None and not custom_source:
        ...
        if engine_def is None:
            ...
            if len(candidates) == 1:
                ...
                console.print()
                console.print(
                    f"[oscat.warning](oscat)[/] '{stripped_answer}' matches multiple engines — "
                ...
                console.print()
                pick = await prompt_or_cancel("(oscat) Enter a number")
                if pick is None:
                    return session
                pick = (pick or "").strip()
                try:
                    ...
                    console.print("[oscat.warning]Invalid choice. Aborting.[/]")
                    console.print()
                    return session
        ...
        if engine_def is None:
            ...
            try:
                console.print()
                console.print("[oscat.muted]        Looking up datasource…[/]")
            ...
            if llm_text.startswith("MATCH:"):
                ...
                if matched_engine is not None:
                    if matched_name.lower() != stripped_answer.lower():
                        confirm = await prompt_or_cancel(
                            f'(oscat) Did you mean "{matched_name}"?',
    ...
    if engine_def.auth_method == "choice" and engine_def.auth_methods:
        console.print()
        console.print(
            f"[oscat.cyan](oscat)[/] How would you like to authenticate with "
        ...
        console.print()
        choice_str = await prompt_or_cancel("(oscat) Enter a number")
    ...
    console.print()
    console.print(
        f"[oscat.cyan](oscat)[/] To connect [bold]{engine_def.display_name}[/], "
    ...
    if required_fields:
        ...
        for f in required_fields:
            console.print(
                f"        • [bold]{f.name:<12}[/] [oscat.muted]— {f.description}[/]"
            )
    ...
    while True:
        mode_answer = await prompt_or_cancel(
            "(oscat) Do you have these available?",
        ...
        console.print(
            "[oscat.warning]        Please enter y, n, or a comma-separated list of parameter names "
    ...
    if mode_answer == "n":
        console.print()
        console.print(
            "[oscat.cyan](oscat)[/] No problem. Which parameters do you have? "
    ...
    for f in fields_to_collect:
        if f.secret:
            ...
            value = await prompt_or_cancel(f"(oscat) {f.name}", default=f.default)
        else:
            value = await prompt_or_cancel(f"(oscat) {f.name}")
        if value is None:
            return session
    ...
    if partial:
        ...
        console.print()
        console.print(
            f"[oscat.muted]Partial connection saved to Local Vault as "
            f'[bold]"{slug}"[/bold]. '
    ...
    if vault.load(engine_def.engine, conn_name) is not None:
        console.print()
        console.print(
            f'[oscat.warning](oscat)[/] A connection [bold]"{slug}"[/bold] already exists.'
        )
        console.print()
        choice = await prompt_or_cancel(
            f"(oscat) {_PROMPT_RECONNECT_CANCEL}",
        )
        ...
        console.print()
        console.print(
            f'[oscat.success]        ✓ Reconnected to [bold]"{slug}"[/bold].[/]'
        )
        console.print()
        session._history.append(
            {
...
async def handle_test_datasource(
    ...
    if not slug:
        console.print(
            "[oscat.warning]Usage: /test <engine-name>[/]"
        )
        console.print()
        return
    ...
    if fields is None:
        console.print(
            f"[oscat.warning]No connection '{slug}' found in Local Vault.[/]"
        )
        console.print()
        return
    ...
    if engine_def is None:
        console.print(
            f"[oscat.warning]Unknown engine '{engine}'. Cannot test.[/]"
        )
        console.print()
        return
    ...
    if not engine_def.test_snippet:
        console.print(
            f"[oscat.warning]No test snippet defined for '{engine}'. Cannot test.[/]"
        )
        console.print()
        return
    ...
    console.print()
    console.print(
        f"[oscat.cyan](oscat)[/] Testing connection [bold]{slug}[/bold]…"
    )
    ...
    if cell is None or cell.error or (
        ...
        console.print()
        console.print(
            f"[oscat.warning](oscat)[/] ✗ Connection test failed for"
            f" [bold]{slug}[/bold]."
        )
        console.print()
        ...
        console.print(
            f"[oscat.success]        ✓ Connection test passed for"
            f" [bold]{slug}[/bold]![/]"
        )
    console.print()
