...
async def handle_resume(
    ...
    if history_store is None:
        console.print("[oscat.warning]History store not available.[/]")
        console.print()
        return session, None
    ...
    if not sessions:
        console.print()
        console.print("[oscat.warning]No previous sessions to resume.[/]")
        console.print()
        return session, None

    console.print()
    console.print("[oscat.cyan]Recent sessions:[/]")
    console.print()

    table = Table(show_header=True, header_style="bold")
    table.add_column("#", style="bold", width=3)
    table.add_column("Date", style="oscat.cyan")
    ...
    choice = await prompt_or_cancel(
        "(oscat) Select session (or q to cancel)", choices=choices, default="q"
    )
    ...
    if history is None:
        console.print("[oscat.error]Failed to load session history.[/]")
        console.print()
        return session, None
    ...
    console.print()
    console.print(
        f"[oscat.success]Resumed session from {selected['date']} ({selected['turns']} turns)[/]"
    )
    console.print()
