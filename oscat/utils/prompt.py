"""Interactive prompt helpers for OSCAT's CLI commands."""
...
MINDS_KEYS = {
    "OSCAT_MINDS_API_KEY",
    "OSCAT_MINDS_URL",
    "OSCAT_MINDS_MIND_NAME",
    "OSCAT_MINDS_DATASOURCE",
    "OSCAT_MINDS_DATASOURCE_ENGINE",
    "OSCAT_MINDS_SSL_VERIFY",
}
...
LLM_KEYS = {
    ...
    "OSCAT_CODING_MODEL",
    "OSCAT_ANTHROPIC_API_KEY",
    "OSCAT_OPENAI_API_KEY",
    "OSCAT_OPENAI_BASE_URL",
}
...
async def prompt_or_cancel(
    ...
    *,
    ...
    from oscat.channel.theme import get_palette as _get_palette
    _prompt_color = _get_palette().prompt

    if label.startswith("(oscat) "):
        body = label[len("(oscat) "):]
        message = HTML(f"<b><style fg='{_prompt_color}'>(oscat)</style></b> {body}{suffix}")
    else:
