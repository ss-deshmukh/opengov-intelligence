...
from oscat.clipboard import clipboard_unavailable_reason
...
def format_file_message(text: str, paths: list[Path], console: Console) -> str:
    ...
    for p in paths:
        ...
        console.print(f"  [oscat.muted]attached: {p.name} ({human_size(size)})[/]")
...
async def ensure_clipboard(console: Console) -> bool:
    ...
    if reason == "unsupported_platform":
        console.print("[oscat.warning]Clipboard is not supported on this platform.[/]")
        return False
    # reason == "missing_pillow"
    console.print("[oscat.muted]Clipboard image support requires Pillow.[/]")
    answer = console.input("[bold]Install Pillow now? (y/n):[/] ").strip().lower()
    if answer not in ("y", "yes"):
        console.print("[oscat.muted]Skipped.[/]")
        return False
    console.print("[oscat.muted]Installing Pillow...[/]")
    import subprocess
    ...
    if proc.returncode == 0:
        console.print("[oscat.success]Pillow installed. Clipboard is now available.[/]")
        return True
    ...
    if proc.returncode == 0:
        console.print("[oscat.success]Pillow installed. Clipboard is now available.[/]")
        return True
    console.print("[oscat.error]Failed to install Pillow.[/]")
    return False
