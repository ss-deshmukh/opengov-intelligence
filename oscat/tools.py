...
CONNECT_DATASOURCE_TOOL = {
    ...
    "description": (
        "Connect a new data source to OSCAT's Local Vault. Call this when the user "
        ...
        "OSCAT will match it to the right connector and guide the user through setup.\n\n"
...
PUBLISH_TOOL = {
    ...
    "description": (
        "Call this after generating an HTML dashboard or report in .oscat/output/. "
    ...
    "input_schema": {
        ...
        "properties": {
            "file_path": {
                "type": "string",
                "description": "Path to the HTML file (e.g. .oscat/output/dashboard.html)",
            },
            "title": {
                "type": "string",
...
SCRATCHPAD_TOOL = {
    ...
    "description": (
        ...
        "All .oscat/.env secrets are available as environment variables (os.environ).\n\n"
...
async def handle_memorize(session: ChatSession, tc_input: dict) -> str:
    ...
    from oscat.memory.hippocampus import Engram
...
async def handle_connect_datasource(session: ChatSession, tc_input: dict) -> str:
    ...
    console.print()
    console.print(
        f"[oscat.prompt]oscat>[/] I can help with that \u2014 let's connect [bold]{engine}[/] to OSCAT."
    )

    from oscat.commands.datasource import handle_connect_datasource
    ...
    if new_connections:
        ...
        console.print()
        console.print("[oscat.muted]  No worries, let's continue where we left off.[/]")
        with Live(
...
async def handle_publish_or_preview(session: ChatSession, tc_input: dict) -> str:
    ...
    if not settings.minds_api_key:
        console.print()
        console.print("  [oscat.muted]To publish you need a free Minds account.[/]")
        console.print("  [oscat.muted]Run [bold]/publish[/bold] to set up your API key and publish.[/]")
        console.print()
        return (
