...
def send_event(settings: OscatSettings, action: str, **extra: str) -> None:
    """Send an analytics event in a background thread.

    Args:
        settings: Resolved OscatSettings (checked for analytics_enabled / analytics_url).
        action: Event name, e.g. ``"oscat_started"``.
