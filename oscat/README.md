...
And here we are: Like an adrenaline junkie eyeing at a bungee looking for another fix, trying again: Meet **OSCAT**.

## A mini Mind

It is probably obvious now, but OSCAT has a brain-inspired architecture, and the more we build it the more it resembles/mirrors functional parts of the brain.  On the other hand we also understand that people don't need to know anything about the brain to play with OSCAT, so we mapped some of the places/files where users can have inputs, or investigate what's up, to names that make more sense than the scientific name of that function of the brain.

The current implementation has three blocks:

| Brain Region                 | Function                                         | OSCAT Equivalent                                              |
|------------------------------|--------------------------------------------------|---------------------------------------------------------------|
...
| **Locus Coeruleus-NE** | Memory modes | The encoding gate. Controls how aggressively OSCAT writes new memories — from broad/indiscriminate to fully suppressed. |
...
```
~/.oscat/                              GLOBAL scope (cross-project)
└── memory/
...
<project>/.oscat/                      PROJECT scope (workspace-specific)
├── memory/
...
├── oscat.md                           User-written project context (unchanged)
└── .env                               Secrets (unchanged)
```

Profile (`profile.md`) is global-only — identity is singular. Rules and lessons exist at both scopes. `oscat.md` stays as the user-written instruction file and is not managed by the memory system.
...
Each session produces one JSONL file in `.oscat/episodes/`:
...
| `assistant` | OSCAT's text response |
...
- **Toggle**: Controlled by `OSCAT_EPISODIC_MEMORY` env var or `/setup` > Memory. Default: ON.
...
The Cortex inserts these as labeled sections in the system prompt (`## Your Memory — Identity`, `## Your Memory — Global Rules`, etc.) so the LLM knows they're its own memories, not user instructions. The `oscat.md` user-written context is injected *after* memory, giving user instructions higher priority.
...
OSCAT has a tool called `memorize` that it can call during conversation to encode new memories. The LLM decides what to save and classifies each entry:
...
- **global** — Universal knowledge useful across any project. Written to `~/.oscat/memory/`.
- **project** — Specific to this workspace. Written to `<project>/.oscat/memory/`.
...
Like the Locus Coeruleus-Norepinephrine system that controls how aggressively the brain writes new memories, OSCAT has three memory modes:

| Mode | Behavior | Brain Analog |
|------|----------|---|
| **autopilot** (default) | OSCAT decides what to save, no confirmation | High tonic NE — broad encoding |
...
Configure via `/setup` > Memory, or the `OSCAT_MEMORY_MODE` environment variable.
...
**Important design rule:** Memory confirmations are *never* shown during scratchpad execution or while OSCAT is composing an answer. They only appear after the user has received their full response, right before the next prompt. This ensures memory never interrupts the workflow.
...
4. Rewrites `~/.oscat/memory/profile.md` atomically (exclusive file lock + write `.tmp` + rename)
...
On first run after upgrading, OSCAT automatically migrates old memory formats:

| Legacy Format | Source | Destination |
|---|---|---|
| `.oscat/context/*.md` | SelfAwarenessContext files | `memory/lessons.md` + `memory/topics/` |
| `.oscat/learnings/*.md` | LearningStore files | `memory/lessons.md` + `memory/topics/` |
...
| `reconsolidate(project_dir)` | Migrate `.oscat/context/` and `.oscat/learnings/` → `.oscat/memory/` |
...
3. ChatSession._build_system_prompt():
   → Calls cortex.build_memory_context()  →  injected before oscat.md
...
10. /setup wizard (sub-menu):
    ...
    → Persisted to OSCAT_MEMORY_MODE and OSCAT_EPISODIC_MEMORY in .oscat/.env
