...
_scratchpad_model = os.environ.get("OSCAT_SCRATCHPAD_MODEL", "")
if _scratchpad_model:
    try:
        import asyncio as _llm_asyncio

        _scratchpad_provider_name = os.environ.get("OSCAT_SCRATCHPAD_PROVIDER", "anthropic")
        if _scratchpad_provider_name in ("openai", "openai-compatible"):
            from oscat.llm.openai import OpenAIProvider as _ProviderClass
        else:
            from oscat.llm.anthropic import AnthropicProvider as _ProviderClass

        _llm_ssl_verify = os.environ.get("OSCAT_MINDS_SSL_VERIFY", "true").lower() != "false"
        if _scratchpad_provider_name in ("openai", "openai-compatible"):
            ...
            _llm_base_url = os.environ.get("OPENAI_BASE_URL") or os.environ.get("OSCAT_OPENAI_BASE_URL")
            _llm_api_key = os.environ.get("OPENAI_API_KEY") or os.environ.get("OSCAT_OPENAI_API_KEY")
...
_minds_datasource = os.environ.get("OSCAT_MINDS_DATASOURCE", "")
_minds_api_key = os.environ.get("OSCAT_MINDS_API_KEY", "")
_minds_url = os.environ.get("OSCAT_MINDS_URL", "")
if _minds_datasource and _minds_api_key and _minds_url:
    try:
        import ssl as _minds_ssl
        import urllib.request as _minds_urllib

        _minds_ssl_verify = os.environ.get("OSCAT_MINDS_SSL_VERIFY", "true").lower() != "false"
        ...
        def query_minds_data(query, datasource=None):
            ...
            req.add_header("User-Agent", "Mozilla/5.0 (compatible; OSCAT/1.0; +https://github.com/mindsdb/anton)")
...
while True:
    ...
    try:
        ...
        if _missing:
            ...
            _uv_path = os.environ.get("OSCAT_UV_PATH", "")
            if _uv_path:
                _pip = _sp.run(
